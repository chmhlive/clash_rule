...
# GeoRouting 基于 GeoIP 的地理位置分流规则

### 项目简述

* **用途：** 根据目的网站的 IP，选取地理位置最近的 VPS 代理节点进行分流（基于 GeoIP 数据库）。
* **原理：** 通过解析目标 IP 所属的国家/地区，将其流量指向对应区域的代理策略组，以降低延迟。
...
1.  **❌ 不建议使用 No_Resolve 版本**：如果不做域名解析，将无法获取 IP 归属地，从而无法充分利用 GeoIP 分流。

2.  **✅ 排序建议 (Final 前)**：建议安排在 Final 兜底规则之前，做最后的分流（以避免影响到其他网站的专属分流控制）。

3.  **✅ 排序建议 (Gfwlist 后)**：建议安排在 Gfwlist 分流之后（以防止敏感域名请求中国大陆 DNS 解析，导致 DNS 泄漏）。

4.  **💡 无法替代 Final**：对于白名单分流来说，本规则集不可替代 Final（因为部分 IP 可能未包含在 GeoIP 库中，仍需 Final 兜底）。
...
⚠️ 注意，仅优先保证 ： 本规则中 “无任何后缀 的规则” ，是长期验证过的（ = 自用级验证 + 100%正确 ） 。其他后缀的规则，均未参与。尤其no_resolve 规则，我本人应该永远也不会 自用级验证，原因请看这里：
...
* **ISO 规范对照表**：[https://www.geonames.org/countries/](https://www.geonames.org/countries/)
* **GeoIP 区域代码**：[https://www.maxmind.com/download/geoip/misc/region_codes.csv](https://www.maxmind.com/download/geoip/misc/region_codes.csv)

> **⚠️ 注意**：洲际编码（Continent Code，即顶级域名）不能直接作为 GeoIP 的索引。例如：`af` 代表非洲（Africa），但作为国家代码时则代表阿富汗（Afghanistan），直接使用会导致重名冲突。

</br>

### 1. 🌎 北美洲 (North America / NA)

</br>

| Geoname ID | Locale | Continent | ISO-3166 Codes | Country Name (En) | 洲际 / 国家名称 (Cn) |
| --- | --- | --- | --- | --- | --- |
